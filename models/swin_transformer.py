...
class CNN_Block(nn.Module):
    def __init__(self, input_ch, output_ch):
        super().__init__()
        self.conv1 = nn.Conv1d(in_channels=input_ch, out_channels=(input_ch + output_ch) // 2, kernel_size=3, padding=1)
        self.norm1 = nn.BatchNorm1d((input_ch + output_ch) // 2)
        self.conv2 = nn.Conv1d(in_channels=(input_ch + output_ch) // 2, out_channels=output_ch, kernel_size=3, padding=1)
        self.norm2 = nn.BatchNorm1d(output_ch)
...
class FinalPatchExpand_X4(nn.Module):
    ...
    def forward(self, x):
        ...
        x = rearrange(x, "b h w (p1 p2 c)-> b (h p1) (w p2) c", p1=self.dim_scale // 2, p2=self.dim_scale,
                      c=C // (self.dim_scale ** 2 // 2))
...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, img_size=(112, 224), patch_size=(2, 4), in_chans=3, embed_dim=96, norm_layer=None):
        super().__init__()
...
class Input_Extracter(nn.Module):
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=1000,
        ...
        print(f"SwinTransformerSys expand initial----depths:{depths};drop_path_rate:{drop_path_rate};num_classes:{num_classes}")
        self.img_size = img_size
        ...
        for i_layer in range(self.num_layers):
            if self.img_size[1] % (8 * 2 ** i_layer) == 0:
                ...
                layer_stop_merge += 1
                layer = BasicLayer(dim=int(embed_dim * 2 ** (i_layer - layer_stop_merge)),
                                   input_resolution=(patches_resolution[0] // (2 ** (i_layer - layer_stop_merge)),
                                                     patches_resolution[1] // (2 ** (i_layer - layer_stop_merge))),
                                   depth=depths[i_layer],
                                   num_heads=num_heads[i_layer],
                                   window_size=window_size,
                                   mlp_ratio=self.mlp_ratio,
        ...
        norm_feature = int(embed_dim * 2 ** (self.num_layers - 1 - layer_stop_merge))
        self.norm = norm_layer(norm_feature)
    ...
    def forward(self, x):
        ...
        for layer in self.layers:
            ...
            x = layer(x)
            # print(f'x shape after {i} layer of encoder:', x.shape)
            i += 1
        x = self.norm(x)  # B L C
...
class MSTI_Block(nn.Module):
    def __init__(self, img_size=(112, 224), patch_size=(2, 4), in_chans=3, num_classes=1000,
        ...
        print(
            f"MSTI_Block expand initial----depths:{depths};drop_path_rate:{drop_path_rate};num_classes:{num_classes}")
...
class MSTO_Block(nn.Module):
    def __init__(self, img_size=(112, 224), patch_size=(2, 4), in_chans=3, num_classes=3,
                 embed_dim=96, depths=[2], num_heads=[3],
                 window_size=7, mlp_ratio=4., qkv_bias=True, qk_scale=None,
                 drop_rate=0., attn_drop_rate=0., drop_path_rate=0.1,
                 norm_layer=nn.LayerNorm, ape=False, patch_norm=True,
        ...
        print(
            f"MSTO_Block expand initial----depths:{depths};drop_path_rate:{drop_path_rate};num_classes:{num_classes}")
        ...
        self.up = FinalPatchExpand_X4(
            input_resolution=(img_size[0] // patch_size[0], img_size[1] // patch_size[1]), dim_scale=4,
            dim=embed_dim)
    ...
    def forward(self, x):
        ...
        if self.final_upsample == "expand_first":
            # print('x shape before up_x4:', x.shape)
            x = self.up(x)
            # print('x shape after up_x4:', x.shape)
            # x = x.view(B,4*H,4*W,-1)
            x = x.view(B, 2 * H, 4 * W, -1)
            x = x.permute(0, 3, 1, 2)  # B,C,H,W
            x = self.output(x)
        # print('x shape after outlayer:', x.shape)
        return x
...
class SwinTransformerSys(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=1000,
        ...
        print(
            f"SwinTransformerSys expand initial----depths:{depths};depths_decoder:{depths_decoder};drop_path_rate:{drop_path_rate};num_classes:{num_classes}")
        ...
        self.IE1 = Input_Extracter(img_size=(56, 112), patch_size=(2, 4), in_chans=3, num_classes=3, embed_dim=48,
                                   ...
                                   mlp_ratio=4., qkv_bias=True, drop_rate=0.0, drop_path_rate=0.2, patch_norm=True,
                                   use_checkpoint=False)
        self.IE2 = Input_Extracter(img_size=(28, 56), patch_size=(2, 4), in_chans=3, num_classes=3, embed_dim=48,
                                   depths=[2, 2, 2, 2], num_heads=[3, 6, 12, 24], window_size=7,
                                   mlp_ratio=4., qkv_bias=True, drop_rate=0.0, drop_path_rate=0.2, patch_norm=True,
                                   use_checkpoint=False)
    ...
    # Encoder and Bottleneck
    def forward_features(self, x, x1, x2, x3):
        x = self.IE0(x)
        x1 = self.IE1(x1)
        x2 = self.IE2(x2)
    ...
    def forward(self, x, x1, x2, x3):
        x = self.forward_features(x, x1, x2, x3)
